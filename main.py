...
@app.get("/nb_transaction_departement/")
...
@app.get("/nb_vente_appartement_plafond-ff/")
...
@app.get("/top_ville_prix_m2_bas/")
async def top_ville_prix_m2_bas(limit:int, type: Type):
    limit = is_number(limit)
    query= f"SELECT ville, AVG(prix/surface_habitable) FROM transactions_sample WHERE UPPER(type_batiment) = UPPER('{type}') GROUP BY ville ORDER BY AVG(prix/surface_habitable) ASC LIMIT '{limit}';"
    return execute_sql_query(con, query)

#12 En tant que CEO, je veux accéder aux 10 villes avec un prix au m2 moyen le plus haut pour les maisons
@app.get("/top_ville_prix_m2_maison_haut")
